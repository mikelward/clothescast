 ...
 * strip: a min→high feels-like thermometer next to the peak-rain droplet. It's
 * the same temperature / rain visual the outfit card (Cast / Home Assistant)
 * draws at the bottom, lifted onto the home screen and laid out horizontally.
 *
 ...
 *
 * First iteration shows temperature + rain only. Wind / UV cells and a
 * current-temp marker are planned follow-ons — the strip renderer already takes
 * discrete per-indicator inputs so adding them is additive.
 */
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        val info = if (insight != null && prefs != null) {
            runCatching {
                ...
                outfitCardInfoLines(context, formatter, insight.hourly, prefs.temperatureUnit)
            }.getOrNull()
        } else {
            null
        }
            ...
            Image(
                provider = ImageProvider(
                    renderConditionsStripBitmap(
                        context = context,
                        rangeLabel = info.tempLine,
                        rainLabel = info.rainLineShort,
                        tempFraction = info.tempFillFraction,
                        rainFraction = info.rainFillFraction,
                        widthPx = widthPx,
                        heightPx = heightPx,
                        darkTheme = darkTheme,
                    ),
...
// widget honours the in-app ThemeMode rather than the device night mode. M3
// light/dark surface neutrals; the red/blue glyph fills read on both.
private val STRIP_LIGHT_SURFACE = Color(0xFFFEF7FF)
private val STRIP_DARK_SURFACE = Color(0xFF1C1B1F)
...
private fun conditionsContentDescription(info: OutfitCardInfoLines): String =
    listOfNotNull(info.tempLine.takeIf { it.isNotBlank() }, info.rainLineShort).joinToString(", ")
