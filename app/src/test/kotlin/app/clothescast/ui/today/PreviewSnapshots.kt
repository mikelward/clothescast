...
import app.clothescast.ui.LauncherIconDevMonochromePreview
...
import app.clothescast.ui.garment.ClothingRainPantsPreview
import app.clothescast.ui.garment.ClothingSnowPantsPreview
...
class PreviewSnapshots {
    ...
    private fun captureUntilStable(maxAttempts: Int = 10, snapshot: () -> Unit) {
        ...
        if (baseline != null) {
            ...
            System.err.println(
                "WARN: snapshot ${testName.methodName} did not stabilise after " +
                    "$maxAttempts attempts; kept the existing baseline",
            )
        } else {
            System.err.println(
                "WARN: snapshot ${testName.methodName} did not stabilise after " +
    ...
    @Test fun seven_day_page_with_location() = capture { SevenDayPageWithLocationPreview() }
    @Test fun seven_day_page_with_per_model_spread_off() = capture { SevenDayPageWithPerModelSpreadOffPreview() }
    ...
    @Test fun wind_card_scrubbed() = capture { WindCardScrubbedPreview() }
    ...
    @Test fun holiday_bank_holiday_with_towel() = capture { HolidayBankHolidayWithTowelPreview() }
    ...
    @Test fun launcher_icon_dev_monochrome() = capture { LauncherIconDevMonochromePreview() }
    ...
    @Test fun clothing_rain_pants() = capture { ClothingRainPantsPreview() }
    @Test fun clothing_snow_pants() = capture { ClothingSnowPantsPreview() }
