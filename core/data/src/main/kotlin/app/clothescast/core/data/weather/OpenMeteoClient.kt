...
class OpenMeteoClient(
    ...
    override suspend fun fetchForecast(location: Location): ForecastBundle = coroutineScope {
        // Primary forecast and the side-band multi-model fetch kick off in
        // parallel — the multi-model call pulls both confidence aggregates and
        // per-model hourly series in one go, so doing it concurrently with the
        // primary fetch hides its latency behind it.
