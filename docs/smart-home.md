...
4. Optionally enter a username + password. The password is stored
   ...
   key uses.
5. Topic prefix defaults to `clothescast/default`. The second segment
   names the target display / group, so if you have multiple Hubs you
   can publish to e.g. `clothescast/kitchen` and `clothescast/bedroom`
   from separate ClothesCast installs. Today's payloads land under
   `<prefix>/today/text`, `<prefix>/today/image`, `<prefix>/today/audio`;
   tonight's mirror those under `<prefix>/tonight/…`. Each period
   publish *also* mirrors to `<prefix>/now/…`, so a single-display
   workflow that just wants "whichever briefing is current" can
   subscribe once to `<prefix>/now/text` (etc.) without time-of-day
   logic of its own.
6. Tap **Save**.
