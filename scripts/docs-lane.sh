...
# and ships as a release-notes bullet, so its commits are legitimately
# bare-subject and belong on the code lane; and anything under a module
# tree (app/, core/) is a build input whatever its extension — a packaged
# asset, or a resource the core JVM modules bundle into their artifacts,
# can be markdown. Everything else is code — including .gitignore, because the
# snapshot-regen step's unforced `git add` cannot stage a newly ignored
# recording, and an ignore rule over app/snapshots/ would silence exactly
# the job the lane skips.
is_housekeeping() {
  case "$1" in
    PRIVACY.md) return 1 ;;
    app/*) return 1 ;;
...
# bullet.
LANE_PREFIXES="ci docs internal test"
