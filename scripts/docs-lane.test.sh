...
check ".gitignore is code (regen git add)" 0 "docs_only=false" FILES=".gitignore README.md"
check "markdown under app/ is a build input" 0 "docs_only=false" FILES="app/src/main/assets/help.md"
check "markdown under core/ is a build input" 0 "docs_only=false" FILES="core/domain/src/main/resources/notes.md"
...
export MODE=gate
ALLOK="unit=success android=success"
ALLSKIP="unit=skipped android=skipped"
...
check "refactor is not in this table"     1 "lacks a housekeeping prefix" FILES="README.md" SUBJECTS="refactor: tidy" CLASSIFY=success RESULTS="$ALLSKIP"
...
check "red job fails the gate"            1 "not all green" FILES="a.kt" CLASSIFY=success RESULTS="unit=failure android=success"
check "half-skipped is not justified"     1 "not all green" FILES="a.kt" CLASSIFY=success RESULTS="unit=skipped android=success"
check "cancelled job fails the gate"      1 "not all green" FILES="a.kt" CLASSIFY=success RESULTS="unit=cancelled android=cancelled"
